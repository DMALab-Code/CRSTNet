...
def load_cfg_from_py(py_path: str) -> EasyDict:
    if py_path.endswith(".py") and os.path.exists(py_path):
        ...
        module_name = "baselines.CRSTNet._parameter_search_config"
        spec = importlib.util.spec_from_file_location(module_name, abs_path)
        if spec is None or spec.loader is None:
            raise RuntimeError(f"Cannot load config from {py_path}")
...
def set_paper_params(cfg: EasyDict, theta: float, lambda_value: float, gamma: float) -> EasyDict:
    ...
    model_param["key_node_ratio"] = float(theta)
    model_param["lambda_spatial"] = float(lambda_value)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Grid search for paper-aligned CRSTNet parameters."
    )
    parser.add_argument("-c", "--config", default="baselines/CRSTNet/PEMS-BAY.py", help="config py path")
    ...
    for theta in args.theta:
        for lambda_value in args.lambda_value:
            for gamma in args.gamma:
                ...
                runner_cls = cfg.get("RUNNER", SimpleTimeSeriesForecastingRunner)
